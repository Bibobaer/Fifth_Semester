...
#define     VM_JE       15
#define     VM_JNE      16
#define     VM_LAST_INS 16
// количество инструкций
#define VM_COUNT_INST   (VM_LAST_INS + 1)

static char *vm_mnem[] = {"nop", "in", "out", "mov", "add", "sub", "xor", "and", "or", "hlt", "push", "pop", "call", "ret", "cmp", "je", "jne"};
...
vm_handler vm_handlers[VM_COUNT_INST] = {vm_nop, vm_in, vm_out, vm_mov, vm_add, vm_sub, vm_xor, vm_and, vm_or, vm_hlt, vm_push, vm_pop, vm_call, vm_ret, vm_cmp, vm_je, vm_jne};
...
static vm_ins_result vm_push (vm_struct *vm) {
    ...
    if (vm_get_operand(vm, &ip->op1, &value) == VM_RESULT_OK) {
        vm_operand temp = { VM_OPTYPE_MEMORY_REG_WORD, VM_REG_SP };
        vm->regs[VM_REG_SP] -= 4;
        vm_set_operand(vm, &temp, value);
    }
...
static vm_ins_result vm_pop (vm_struct *vm) {
    ...
    vm_instruction* ip = vm_get_current_instruction(vm);
    vm_set_operand(vm, &ip->op1, *(vmopvalue_t*)(vm->memory + vm->regs[VM_REG_SP]));
    vm->regs[VM_REG_SP] += 4;
    return VM_RESULT_OK;
}
...
static vm_ins_result vm_call(vm_struct* vm) {
    vmopvalue_t value;
    vmopvalue_t next_instruction_addr = (vmopvalue_t)(vm->ip) + VM_INSTRUCTION_SIZE;

    vm_instruction* ip = vm_get_current_instruction(vm);
    if (vm_get_operand(vm, &ip->op1, &value) == VM_RESULT_OK) {
        vm->regs[VM_REG_SP] -= 4;
        *((vmopvalue_t*)(vm->memory + vm->regs[VM_REG_SP])) = next_instruction_addr;

        vm->ip = (vm_instruction*)(value);
        return VM_RESULT_CHANGE_IP;
    }
    return VM_RESULT_OK;
}

static vm_ins_result vm_ret(vm_struct* vm) {
    vmopvalue_t code_addr;

    code_addr = *((vmopvalue_t*)(vm->memory + vm->regs[VM_REG_SP]));
    vm->regs[VM_REG_SP] += 4;
    vm->ip = (vm_instruction*)(code_addr);
...
static vm_ins_result vm_cmp(vm_struct* vm) {
    ...
    if (vm_get_operand(vm, &ip->op1, &value1) == VM_RESULT_OK &&
        vm_get_operand(vm, &ip->op2, &value2) == VM_RESULT_OK) {
        value1 -= value2;
        vm->flags = value1 == 0 ? (vm->flags | 1) : (vm->flags & ~1);
    }

    return VM_RESULT_OK;
}// 00001000 01011 & 1111 1110

static vm_ins_result vm_je(vm_struct* vm) {
    vmopvalue_t value;
    vm_instruction* ip = vm_get_current_instruction(vm);
    if (vm_get_operand(vm, &ip->op1, &value) == VM_RESULT_OK && vm->flags & 0x1) {
        vm->ip = (vm_instruction*)(value);
        return VM_RESULT_CHANGE_IP;
    }
    return VM_RESULT_OK;
...
vm_ins_result vm_run_current_instruction (vm_struct *vm) {
    ...
    if (res == VM_RESULT_OK || res == VM_RESULT_BREAK) { vm->ip++;  }
    else if (res == VM_RESULT_CHANGE_IP) {}
    else { return res; }

    vm->regs[VM_REG_IP] = (vmopvalue_t)vm->ip;
...
static void vm_init (vm_struct *vm) {
    ...
    vm->regs[VM_REG_IP] = (vmopvalue_t)(VM_CODE_START);
    vm->regs[VM_REG_SP] = (vmopvalue_t)(VM_STACK_START);
...
BOOL vm_load_code (vm_struct *vm, unsigned char *code, unsigned int code_size) {
    ...
    if (code_size < VM_MEM_SIZE - VM_CODE_START) {
        ...
        vm->regs[VM_REG_COUNT-1] = VM_STACK_START;// stack pointer
        return TRUE;
        }
